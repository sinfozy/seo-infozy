...
export default function TableData({
  ...
  const handleStatusChange = (id: string, currentStatus: boolean) => {
    ...
    update();
    data.find((item) => item._id === id)!.isActive = !currentStatus;
  };

  const handleViewDetails = (id: string) => {
    router.push(`/admin/dashboard/${label.toLocaleLowerCase()}s/${id}`);
  };

  const handleCreate = () => {
    router.push(`/admin/dashboard/${label.toLocaleLowerCase()}s/new`);
  };
              ...
              <TableRow key={row._id}>
                ...
                {columnVisibility.reseller && (
                  <TableCell>
                    {row.resellerId ? (
                      <Link
                        href={`/admin/dashboard/resellers/${row.resellerId?._id}`}
